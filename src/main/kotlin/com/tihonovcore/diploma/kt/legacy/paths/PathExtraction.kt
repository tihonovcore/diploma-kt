...
import com.tihonovcore.diploma.kt.legacy.isNotTooBig
import com.tihonovcore.diploma.kt.legacy.kind
import com.tihonovcore.diploma.kt.legacy.toDatasetStyle
...
fun createSampleForFit(
    ...
    val samples = buildTree(root, null)
        .addAfterLast()
        .elementsFromDepth(depth)
        /* @doubt: don't use it here. it is probably better to take single random node */
        .smartlyTake(samplesCount)
        .map { targetElement ->
            val targetIndex = targetElement.parent!!.children.indexOf(targetElement)
...
//    return samples
//        .filter { it.first.target != AFTER_LAST_KIND && it.first.isNotTooBig() }[20]
//        .also { println(it.first.rootPath); println(it.first.target) }
}
...
private fun AstWithAfterLast.elementsFromDepth(depth: Int): List<AstWithAfterLast> {
    ...
    /* @suggest
    return children.flatMap { it.elementsFromDepth(depth - 1) }
    */
}
...
//    return rankedMerge.take(samplesCount)
    /* @suggest: comment */
    return filterNot { it.original === AfterLast }
}
